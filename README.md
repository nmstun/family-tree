...
- 📸 **写真付きメンバー管理** - 各人物の写真を登録・編集・削除可能（登録時・登録済みの写真ともトリミング可）。名前検索付きの一覧で数十人規模でも探しやすい。一覧と家系図のサイドパネルに出ている丸いアイコンを押すと写真を大きく表示する（アイコンは数十pxしかなく顔が判別できないため）。Escキー・背景クリック・×のいずれでも閉じられる

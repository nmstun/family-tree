...
- 🖨️ **PDF出力・印刷** - 「PDF・印刷」ボタンからブラウザの印刷ダイアログを開き、「PDFとして保存」を選ぶとPDFになる。家系図はSVG（ベクター）のまま出力されるため、縮小しても文字が潰れない。用紙サイズは家系図の縦横比に合わせて自動で決まるので、余白がほとんど出ない（家系図は縦横比が極端になりやすく、A4に固定すると半分近くが余白になるため）。家系図以外の画面要素と折りたたみボタンは出力されない

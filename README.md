...
- 📍 **子を持てなかった親の配置** - 兄弟2人がよその姉妹2人と結婚しているような場合、どちらの子夫婦も片方の親にしかぶら下げられず、負けたほうの親は子を1つも持たないまま図の端へ流れてしまう（実データで子への線が3696px＝図幅の65%まで伸びていた）。世代は同じなので、実際に子を抱えた親の隣に並べる。実データではその線が352pxと176pxになり、図全体の幅も5668→5140pxに縮んだ
...
- 🖱️ **ノードを選んで編集** - 家系図表示でノードをクリックすると、その人の詳細がサイドパネル（スマホでは下から出るシート）に開く。氏名・生没年・写真などの属性編集に加え、配偶者・親・子の追加と削除、「自分」の設定、メンバーの削除がその場でできる。関係の一覧に出ている名前を押すとその人に切り替わるので、家系図をたどりながら続けて編集できる。関係の追加時の検証（重複・自分自身・循環）は「関係」タブと同じものを使う
...
npm test
# ├ treeLayout      … 線の重なり・世代の間隔などの表示崩れ、長い線の色分け、
#                     子を持てなかった親の配置
...
│   │   ├── MemberDetailPanel.tsx # 家系図で選んだ人の属性・関係を編集するパネル
...
│   │   ├── memberDeletion.ts     # 削除の影響（巻き添えになる関係）の集計
│   │   ├── memberName.ts         # 氏名・頭文字の表示
│   │   ├── relationValidation.ts # 配偶者・親子関係の入力チェック

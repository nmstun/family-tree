...
- 📸 **写真付きメンバー管理** - 各人物の写真を登録・編集・削除可能（登録時・登録済みの写真ともトリミング可）。トリミングは**正方形・縦長(3:4)・横長(4:3)**から選べる（縦長の顔写真が正方形に切り落とされるのを避けるため）。縮小して切り抜き枠を元画像より大きくすると、はみ出したぶんは白い余白になる。名前検索付きの一覧で数十人規模でも探しやすい。一覧と家系図のサイドパネルに出ている丸いアイコンを押すと写真を大きく表示する（アイコンは数十pxしかなく顔が判別できないため）。Escキー・背景クリック・×のいずれでも閉じられる
...
- 📍 **子を持てなかった親の配置** - 兄弟2人がよその姉妹2人と結婚しているような場合、どちらの子夫婦も片方の親にしかぶら下げられず、負けたほうの親は子を1つも持たないまま図の端へ流れてしまう（実データで子への線が3696px＝図幅の65%まで伸びていた）。世代は同じなので、実際に子を抱えた親の隣に並べる。実データではその線が422pxと106pxになり、図全体の幅も5668→5140pxに縮んだ。隣に置いた親の縦線が、下の世代のカードの真上（＝そのカードへ別の親から下りている縦線と同じ位置）に来る場合は、重なって1本に見えてしまうため横へ逃がす
...
- 🖱️ **ノードを選んで編集** - 家系図表示でノードをクリックすると、その人の詳細がサイドパネル（スマホでは下から出るシート）に開く。氏名・生没年・写真などの属性編集に加え、配偶者・親・子の追加と削除、「自分」の設定、メンバーの削除がその場でできる。関係の一覧に出ている名前を押すとその人に切り替わるので、家系図をたどりながら続けて編集できる。関係の追加時の検証（重複・自分自身・循環）は「関係」タブと同じものを使う。編集フォームを開いている間は家系図のノードを選べない（選択が変わるとフォームを閉じるため、入力中の内容が消えてしまう）
...
# ├ cropImage      … 写真の切り抜き（縦横比を保つこと・はみ出しを白余白にすること）
...
├── src/
│   ├── components/
│   │   ├── ui/                   # 共通UI部品（Button/Card/Field/Alert/Modal/確認ダイアログ/写真の拡大表示）
...
│   │   ├── cropImage.ts          # 写真の切り抜き（縦横比の保持・白余白）
